...
Модуль `cs2bot.match_sources` получает нормализованные результаты из документированного PandaScore API и использует одобренный LiquipediaDB API как резервный и диагностический источник. В shadow-режиме Liquipedia сравнивается с PandaScore, но не меняет публикацию. Fallback включается отдельно только при пустом, невалидном, недоступном или устаревшем ответе PandaScore. Перед публикацией handler проверяет качество и свежесть данных, сохраняет результат в durable outbox и резервирует доставку атомарным Object Storage claim. Непосредственно перед внешним запросом claim становится неперехватываемым `attempting`; после подтверждения платформа фиксируется как `sent`, создаётся processed marker и удаляется элемент outbox. Если запись marker прервётся, следующий запуск восстановит его без повторной отправки.
...
| `SOCIAL_PROXY_URL` | Необязательный URL HTTP(S)-прокси отдельной OAuth-функции для запросов к Meta. В production передавайте только через Lockbox. |
...
| `ENABLE_INSTAGRAM_PUBLISHING` | `1` включает независимую доставку карточек в Instagram; по умолчанию `0`. |
...
Обычный claim живёт `DELIVERY_CLAIM_TTL_SECONDS`. Непосредственно перед внешним
publish-запросом он атомарно переходит из `sending` в неперехватываемое состояние
`attempting`. Подтверждённая отправка переводит его в неперехватываемый `sent`
для восстановления отсутствующего processed marker; значение `expires-at` через
24 часа остаётся диагностической меткой и само не разрешает повтор.
Неоднозначный ответ переводит claim в `uncertain`, без автоматического повтора.
Outbox читает все страницы Object
Storage, затем выбирает новые и наименее недавно обработанные элементы, поэтому
лексикографически поздние ключи не голодают.

Для результата Telegram загрузка PNG делает до двух безопасных попыток при
`ConnectTimeout`, после чего в том же invocation выполняется одна текстовая
попытка и на час включается text-only режим. Если TCP-соединение не установилось
и для текста, claim освобождается, а outbox сохраняется для следующего timer
...
- `job=schedule`: один утренний выпуск только с матчами Tier-1 турниров.
...
- `job=radar_discovery`: ежедневный автоматический поиск Tier-1 турниров,
  чей самый ранний матч по PandaScore приходится на следующий календарный день;
  уже начавшиеся турниры пропускаются, а публикация выполняется один раз на
  tournament ID и канал только при наличии подтверждённых пар сетки.
...
qualifier, showmatch, academy, youth и junior. Для результатов эти поля остаются
диагностикой: там действует более строгий Tier-1 LAN-фильтр. Список tiers задаётся
в `tier1_autopilot_tiers` фильтр-конфига (по умолчанию `s`, `a`).
...
После расписания бот формирует контекст ко всем отобранным матчам: форму за пять
завершённых серий и очные встречи за три месяца. Матчи группируются по турнирам,
а длинный контекст безопасно разбивается между матчами на несколько сообщений.
Ошибка контекста не блокирует основной выпуск и не создаёт отдельный retry.

При `TELEGRAM_MEDIA_CARDS=1` результаты отправляются как квадратные PNG со
спойлером на всём изображении, а счёт в подписи остаётся скрыт через
`<tg-spoiler>`; отдельная строка с победителем в подписи не добавляется.
Вечерний итог поддерживает квадратную адаптивную карточку от одного до десяти
матчей. В Telegram сначала выходит полноценная карточка расписания с парами и
временем начала (при необходимости — альбом). После подтверждения её доставки
отдельно выходит контекстная обложка на каждый турнир и формат; короткий
одиночный контекст прикрепляется к ней подписью. Контекст не заменяет расписание,
а сбой его доставки не вызывает повтор уже опубликованного расписания.
Instagram и Threads независимо получают полноценные карточки расписания:
1–10 матчей на одной странице, 11–20 — в двухстраничной карусели. Отключённый
`TELEGRAM_MEDIA_CARDS` не мешает соцсетевым карточкам и не включает изображения
в Telegram. При ошибке генерации или доставки бот возвращается к текстовому
расписанию. Во всех шаблонах логотип канала расположен по центру верхней части
карточки. Если логотип команды отсутствует,
...
`GET /tournaments?filter[id]=...`. Tier `S`/`A` участвует в production-отборе
утреннего расписания после жёстких исключений. В потоке результатов tier остаётся
диагностическим сигналом и не заменяет строгий Tier-1 LAN-фильтр.

Для утреннего расписания используется документированный endpoint `GET /csgo/matches/upcoming`. Окно запроса соответствует текущему календарному дню в `DISPLAY_TIMEZONE`. В расписание попадают только Tier-1 турниры; qualifiers, showmatch, мелкие турниры и молодёжные составы исключаются даже при ошибочной высокой tier-метке PandaScore.
