...
Модуль `cs2bot.match_sources` получает нормализованные результаты из документированного PandaScore API и использует одобренный LiquipediaDB API как резервный и диагностический источник. В shadow-режиме Liquipedia сравнивается с PandaScore, но не меняет публикацию. Fallback включается отдельно только при пустом, невалидном, недоступном или устаревшем ответе PandaScore. Перед публикацией handler проверяет качество и свежесть данных, резервирует доставку атомарным Object Storage claim, после подтверждения Telegram фиксирует состояние `sent`, а затем создаёт processed marker. Если запись marker прервётся, следующий запуск восстановит его без повторной отправки.
