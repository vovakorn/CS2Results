...
через `<tg-spoiler>`. Расписание до шести матчей отправляется как вертикальная
карточка. Если логотип отсутствует, используется аккуратный плейсхолдер с
инициалами. Если карточку нельзя сформировать или доставить, бот автоматически
возвращается к существующему текстовому сообщению и не теряет матч.
