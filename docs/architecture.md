...
5. Telegram сначала получает карточки расписания (либо текст при отключённых
   карточках или безопасном fallback). После подтверждения основной публикации
   claim фиксируется как `sent`, затем отправляются отдельные обложки и сообщения
   контекста. Сбой контекста не вызывает повтор расписания; контекст никогда не
   подставляется вместо основной карточки или её подписи.
